...
import {computeSigningRoot} from "@lodestar/light-client/domain";
import {getLcLoggerConsole} from "@lodestar/light-client/logger";
...
export const SLOTS_PER_PERIOD = EPOCHS_PER_SYNC_COMMITTEE_PERIOD * SLOTS_PER_EPOCH;
export const SOME_HASH = Buffer.alloc(32, 0xaa);
const interopSyncCommitteeCache = new Map<SyncPeriod, SyncCommitteeKeys>();
...
export function getInteropSyncCommittee(period: SyncPeriod): SyncCommitteeKeys {
  const cachedSyncCommittee = interopSyncCommitteeCache.get(period);
  if (cachedSyncCommittee) {
    return cachedSyncCommittee;
  }

  const skBytes = Buffer.alloc(32, 0);
  skBytes.writeInt32BE(1 + period, 0);
  ...
  const syncCommittee: SyncCommitteeKeys = {
    pks,
    signAndAggregate,
    signHeader,
    syncCommittee: {
  ...
  interopSyncCommitteeCache.set(period, syncCommittee);
